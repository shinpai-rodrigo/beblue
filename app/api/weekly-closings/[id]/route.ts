...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Only ADMIN can reopen a FECHADO or CONCILIADO closing
    if ((existing.status === 'FECHADO' || existing.status === 'CONCILIADO') && parsed.data.status === 'ABERTO') {
      if (session.role !== 'ADMIN') {
        return NextResponse.json(
          { success: false, error: 'Apenas administradores podem reabrir um fechamento concluído' },
          { status: 403 }
        );
      }
    }
