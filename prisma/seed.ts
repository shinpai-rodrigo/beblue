import { PrismaClient, Role, ClientType, CommissionBasis, ReimbursementCategory, CampaignStatus, InvoiceStatus, ReimbursementStatus, InfluencerPaymentStatus, CommissionStatus } from '@prisma/client';
...
async function main() {
  ...
  // In production without explicit passwords, don't overwrite existing passwords on update
  const userUpdateData = (isProduction && !hasExplicitPasswords)
    ? { active: true }
    : { password: hashedPassword, active: true };

  const regularUpdateData = (isProduction && !hasExplicitPasswords)
    ? { active: true }
    : { password: hashedUser, active: true };
  ...
  // Check if rest of seed data exists - if so, skip to avoid duplicates
  const existingEmployees = await prisma.employee.count();
  if (existingEmployees > 0) {
    console.log('Seed data already exists (employees found). Passwords updated, skipping rest.');
    return;
  }
