...
# NOTE: This container runs as root because start.sh must:
#   1. Initialize and start PostgreSQL (requires chown/su to postgres user)
#   2. Create the database and user via `su - postgres`
# The PostgreSQL process runs as the 'postgres' user (via su).
# The Next.js server is started via `exec node` which inherits root.
# In a production-grade setup, PostgreSQL should be a separate service
# and this container would run as a non-root user.
CMD ["/app/start.sh"]
